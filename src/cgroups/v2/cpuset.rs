...
mod tests {
    ...
    fn test_set_cpus() {
        ...
        // assert
        let content = fs::read_to_string(&cpus).expect(&format!("read {} file content", CGROUP_CPUSET_CPUS));
    ...
    fn test_set_mems() {
         ...
         // assert
         let content = fs::read_to_string(&mems).expect(&format!("read {} file content", CGROUP_CPUSET_MEMS));
         assert_eq!(content, "1-3");
    }
}
