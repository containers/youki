...
impl HugeTlb {
    ...
    fn stats_for_page_size(
        ...
        let mut rsvd = ".rsvd";
        let mut path = cgroup_path.join(format!("hugetlb.{page_size}{rsvd}.events"));
        let events = read_cgroup_file(&path).or_else(|_| {
            rsvd = "";
            path = cgroup_path.join(format!("hugetlb.{page_size}{rsvd}.events"));
        ...
        Ok(HugeTlbStats {
            usage: parse_single_value(
                &cgroup_path.join(format!("hugetlb.{page_size}{rsvd}.current")),
            )?,
            fail_count,
            ..Default::default()
        })
